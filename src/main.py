...
from pipeline_visualizer.renders import standard_render, warp_render
...
def pipeline(
    ...
    *,
    ...
    layout: Literal["panel", "warp"] = "panel",
) -> None:
    ...
    Args:
        ...
        layout (Literal["panel", "warp"]): Layout style of the visualization.
    """
    parsed_pipeline = standard_parser.parse(cmd)

    match layout:
        case "panel":
            standard_render.render(parsed_pipeline, arrow_style=arrow)
        case "warp":
            warp_render.render(parsed_pipeline, arrow_style=arrow)
